...
export function createHeroPanel(scene: Phaser.Scene, opts: HeroPanelOptions): Phaser.GameObjects.Container {
  ...
  // Stats 2×2 grid
  const stats = [
    { key: "stat_hp", val: `${hero.currentHp ?? hero.maxHp}/${effMaxHp}` },
    { key: "stat_atk", val: effAtk },
    { key: "stat_def", val: effDef },
    { key: "stat_mag", val: effMag },
  ];
  const col1X = cx - 72;
  const col2X = cx + 36;
  stats.forEach((s, i) => {
    ...
    add(scene.add.text(sx + 18, sy, `${s.val}`, { fontSize: FONT_BODY, color: TXT_GOLD_LIGHT }).setOrigin(0, 0.5));
  });

  // Gold
  add(scene.add.text(cx, panelY + 230, `Gold: ${hero.gold ?? 0}`, {
    fontSize: FONT_BODY, fontFamily: "EnchantedLand", color: TXT_GOLD,
  }).setOrigin(0.5));

  // XP bar
  const xpPct = Math.min(1, hero.xp / xpToNextLevel);
  add(scene.add.text(cx, panelY + 254, `XP  ${hero.xp} / ${xpToNextLevel}`, {
    fontSize: FONT_SM, color: TXT_GOLD_MID,
  }).setOrigin(0.5));
  add(scene.add.rectangle(panelX + 10, panelY + 270, barW, 11, BG_ROW_MID).setOrigin(0));
  add(scene.add.rectangle(panelX + 10, panelY + 270, barW * xpPct, 11, BAR_XP_FILL).setOrigin(0));

  // Divider
  add(scene.add.rectangle(cx, panelY + 294, barW, 1, BORDER_GOLD, 0.6).setOrigin(0.5));

  // Equipped moves header
  add(scene.add.text(cx, panelY + 312, "Equipped Moves", {
